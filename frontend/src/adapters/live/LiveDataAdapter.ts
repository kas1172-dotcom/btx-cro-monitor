...
export class LiveDataAdapter implements DataAdapter {
  ...
  async getOperatingSnapshot(): Promise<OperatingSnapshot> {
    ...
    return {
      ...
      pipeline: {
        source_type: "demo",
        source_name: status.provenance ?? "Live CRM",
        ...
        summary: {
          ...
          priority_accounts: [],
          top_action: status.errors.length ? "Resolve CRM connection errors." : "Live CRM data is connected.",
        },
        records: [],
      },
      integrations: [{
        ...
        description: status.errors.length ? status.errors[0] : "Live account, contact, and deal reads are served from the configured CRM.",
        is_demo: false,
      }],
      assumptions: {
        source_type: "demo",
        ...
        summary: status.errors.length ? "Live CRM reads are currently unavailable." : "CRM records are read live from the configured adapter.",
      ...
      publicSignals: {
        ...
        source_mode: "static_snapshot",
        notice: status.errors.length ? status.errors.join(" ") : "Live: CRM",
      },
    };
  }
}
