...
export function recordToDeliverable(record: DeliverableRecord): Deliverable {
  return {
    ...record.document,
...
export async function listDeliverableRecords(): Promise<DeliverableRecord[]> {
  const response = await backendJson<DeliverableListResponse>("/deliverables");
  return response.records;
}

export async function getDeliverableRecord(id: string): Promise<DeliverableRecord> {
  return backendJson<DeliverableRecord>(`/deliverables/${encodeURIComponent(id)}`);
}

export async function createDeliverableRecord(deliverable: Deliverable): Promise<DeliverableRecord> {
  return backendJson<DeliverableRecord>("/deliverables", {
    method: "POST",
    body: JSON.stringify({
...
export async function patchDeliverableRecord(recordId: string, deliverable: Deliverable): Promise<DeliverableRecord> {
  return backendJson<DeliverableRecord>(`/deliverables/${encodeURIComponent(recordId)}`, {
    method: "PATCH",
    body: JSON.stringify({
      type: deliverable.type,
      title: deliverable.title,
...
export async function saveDeliverableRecord(deliverable: Deliverable): Promise<DeliverableRecord> {
  if (deliverable.backendRecordId) return patchDeliverableRecord(deliverable.backendRecordId, deliverable);
  return createDeliverableRecord(deliverable);
}
