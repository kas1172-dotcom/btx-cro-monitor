...
export function buildDocxDocument(deliverable: Deliverable, docx: DocxModule) {
  ...
  const children: Array<ReturnType<typeof docxBlock>[number]> = [
    new Paragraph({
      style: "BTXBrand",
      children: [new TextRun({ text: ASSISTANT_BRAND_NAME, bold: true, color: DOCX_COLORS.teal, size: 20, font: "Arial" })],
    }),
    new Paragraph({
  ...
  return new Document({
    ...
    title,
    description: `${PROFILE.name} deliverable generated by ${ASSISTANT_BRAND_NAME}`,
    styles: {
      default: {
        document: {
...
export function downloadIcs(deliverable: Deliverable): void {
  ...
  downloadFile(`${slugTitle(deliverable)}.ics`, ["BEGIN:VCALENDAR", "VERSION:2.0", "PRODID:-//BTX//Enterprise Brain//EN", ...events, "END:VCALENDAR"].join("\r\n"), "text/calendar;charset=utf-8");
}
