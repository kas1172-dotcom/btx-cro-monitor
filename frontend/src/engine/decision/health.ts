...
export function pipelineHealth(opps: Opportunity[]): number | null {
  const relevant = opps.filter((o) => o.stage in STAGE_WEIGHT || o.stage === "won" || o.stage === "lost");
...
export function healthLabel(score: number | null): string {
  if (score === null) return "no pipeline yet";
