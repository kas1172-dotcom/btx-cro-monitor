...
async function loadWorld(city: string | null = null): Promise<World> {
  ...
  return {
    ...
    snapshot,
    worldSnapshot: {
      generatedAt: snapshot.pipeline.as_of,
      tenant: { id: "tour-demo", displayName: "Tour demo", isDemonstration: true, demoNotice: "Demonstration — internal records are illustrative" },
      accounts: companies,
      contacts,
      facilities,
      opportunities,
      signals: [...rawSignals, ...newsSignals],
      scores: null,
      workItems: [],
      sourceHealth: [],
    } as World["worldSnapshot"],
    scoreResults: null,
    dataSource: null,
    loadErrors: [],
    provenanceSources: [],
