...
def seed_engine_configs(session_factory: sessionmaker[Session]) -> None:
    with session_factory() as session:
        existing = {
            row[0] for row in session.execute(select(models.EngineConfig.name).distinct()).all()
        }
        ...
        for name, document in seeds.items():
            ...
            logger.info("engine_config.seed", extra={"config_name": name})
        session.commit()
...
def latest_config(session: Session, name: str) -> models.EngineConfig | None:
    return session.execute(
        select(models.EngineConfig)
        .where(models.EngineConfig.name == name)
        .order_by(models.EngineConfig.version.desc())
        .limit(1)
    ).scalar_one_or_none()


def config_history(session: Session, name: str, limit: int = 20) -> list[models.EngineConfig]:
...
def put_config(session: Session, name: str, document: dict[str, Any], change_note: str | None) -> models.EngineConfig:
    validated = validate_config_document(name, document)
    latest = latest_config(session, name)
    ...
    session.commit()
    session.refresh(row)
    logger.info("engine_config.put", extra={"config_name": name, "version": next_version})
    return row
